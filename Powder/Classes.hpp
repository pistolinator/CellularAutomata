...
public:
    ...
    // clamps the magnitude of the vector to maxLength if its current magnitude exceeds maxlength
    void CapMagnitude (float maxLength) { // could be optimized
        float magnitude = getMagnitude ();
    ...
    // returns 4-way int direction of magnitude 1
    IntVector2 toIntDir () {
        IntVector2 result = IntVector2 (0, 0);
        
        if (NumMagnitude (x) >= NumMagnitude (y)) {
            result.x = SignOf (x);
        } else {
            result.y = SignOf (y);
        }
    ...
    static float NumMagnitude (float num) {
        return num * SignOf (num);
    }
    // returns real euclidean remainder; performs modulo operation correctly on negative numbers
    static int MathMod (int a, int b) {
        int r = a % b;
        return r >= 0 ? r : r + abs(b);
    }
    static float MathMod (float af, int b) {
        int a = af;
        int r = a % b;
        
        float result = r >= 0 ? r : r + abs(b);
        float decimals = af - a;
        result += decimals;
        
        return result;
    }
    // returns the given num with the smallest magnitude ... with the sign of the first number
    static float getSmallest (float a, float b) {
        float result = abs(b) * SignOf2 (a);
        if (abs (a) <= abs(b)) {
            result = a;
        }
        return result;
