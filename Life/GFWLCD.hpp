...
public:
    ...
    bool TakeScreenshot (string filename) {
        ...
        int xOrigin = 78 / 2;
        int yOrigin = 46 / 2;
        int width = 320 * ZOOM_SCALE;
        int height = 240 * ZOOM_SCALE + 22;
        
        string command = "screencapture -R " + to_string (xOrigin) + "," + to_string (yOrigin) + "," + to_string (width) + "," + to_string (height) + " " + filename;
        char const *charCommand = command.data ();
        system (charCommand);
        return true;
    }
    // takes cropped screenshot of screen; excludes the top menu bar and stuff at sides
    bool TakeCroppedScreenshot (string filename) {
        // string command = "screencapture -R 78,46,320,240 " + filename;
        int STUFF_AT_SIDES_SIZE = 3; // in pixels
        int xOrigin = 78 / 2 + STUFF_AT_SIDES_SIZE;
        int yOrigin = 46 / 2 + 22;
        int width = 320 * ZOOM_SCALE - 2 * STUFF_AT_SIDES_SIZE;
        int height = 240 * ZOOM_SCALE - STUFF_AT_SIDES_SIZE;
